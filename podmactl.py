    ...
    def _call(self, *args: str, **kwargs) -> str:
        ...
        with subprocess.Popen(
            ...
            **kwargs,
            ...
            stdout_orig: Iterable[bytes] = process.stdout or []
    ...
    def update(self, machine: Machine):
        ...
        if machine.qemu_binary:
            ...
            if not (cmd_line := config.get("CmdLine")):
                logger.error("Cannot find CmdLine in config of %s at", machine.name, config_path)
...
def main(
    ...
    active_machines = [name for name, machine in requested_machines.items() if machine.active]
    ...
    logger.info("%s is active and podman is ready to be used", requested_active)
...
def MachineDict(json_path: str) -> dict:  # noqa: N802
    try:
...
if __name__ == "__main__":
    parser = ArgumentParser("podman-launchd", description="CRUDs pod machines and starts one")
    ...
    parser.add_argument("-p", "--podman", help="Name or path of the podman command to use")
    parser.add_argument("--verbose", help="Activate verbose logging", action="store_true")
